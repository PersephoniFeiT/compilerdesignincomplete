...
typedef struct SymbolTable {
    ...
    ActivationStack* scopeStack;
} SymbolTable;

int hash(char* id);
int put(SymbolTable* env, char* id, TypeRecord val);
...
int put(SymbolTable* env, char* id, TypeRecord* val){
    ...
    ActivationRecord currentRecord = peekActivationRecord(env->scopeStack);
    ActivationRecord newRecord = {0, val, getCurrentOffset(env->scopeStack)};
    newBinding->memAddress = getBaseAddress(env->scopeStack);
    pushActivationRecord(env->scopeStack, newRecord);
    return getCurrentOffset(env->scopeStack);
}

int putTempBinding(SymbolTable* env, Binding* tempBinding){
    ...
    ActivationRecord currentRecord = peekActivationRecord(env->scopeStack);
    ActivationRecord newRecord = {0, tempBinding, getCurrentOffset(env->scopeStack)};
    tempBinding->memAddress = getBaseAddress(env->scopeStack);
    pushActivationRecord(env->scopeStack, newRecord);
    return getCurrentOffset(env->scopeStack);
}
...
SymbolTable* constructTable(int size, SymbolTable* prev){
    ...
    newTable->prev = prev;
    newTable->scopeStack = generateActivationStack(prev != NULL ? prev->scopeStack : NULL);
}
...
void pushActivationRecord(ActivationStack* stack, ActivationRecord record){
    stack->records = realloc(stack->records, sizeof(ActivationRecord) * (stack->top + 1));
    stack->records[stack->top] = record;
    stack->top += 1;
}

ActivationRecord popActivationRecord(ActivationStack* stack){
    if(stack->top == 0){
        ActivationRecord emptyRecord;
        emptyRecord.controlLink = -1;
        emptyRecord.localData = NULL;
        emptyRecord.offset = -1;
        return emptyRecord;
    }
    stack->top -= 1;
...
ActivationRecord peekActivationRecord(ActivationStack* stack){
    if(stack->top == 0){
        ActivationRecord emptyRecord;
        emptyRecord.controlLink = -1;
        emptyRecord.localData = NULL;
    ...
    return stack->records[stack->top - 1];
}

long int getCurrentOffset(ActivationStack* stack){
    ActivationRecord currentRecord = peekActivationRecord(stack);
    return currentRecord.offset;
}

int getCurrentAddrLength(ActivationStack* stack){
    ActivationRecord currentRecord = peekActivationRecord(stack);
    TypeRecord* bindingType = currentRecord.localData->value;
    return sizeOfType(*bindingType);
}

long int getBaseAddress(ActivationStack* stack){
    return getCurrentOffset(stack) + getCurrentAddrLength(stack);
}

ActivationStack* generateActivationStack(ActivationStack* prevStack){
    if(prevStack == NULL){
        ActivationStack* newStack = (ActivationStack*) malloc(sizeof(ActivationStack));
        newStack->top = 0;
        newStack->records = NULL;
        return newStack;
    }
    ActivationStack* newStack = (ActivationStack*) malloc(sizeof(ActivationStack));
    newStack->top = &prevStack->top;
    newStack->records = &prevStack->records;
    return newStack;
}